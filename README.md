...
| **본인** | 그 외 로직 전체 — Retriever 3종 설계·구현, Self-Query 메타데이터 설계, LLM 라우터 체인, RAG 파이프라인 전체, 백엔드 |
...
> 아래는 **원본 실행 로그 발췌**입니다. 저장소의 노트북은 개인정보 보호를 위해 출력을 지운 상태로 올렸습니다.
...
| 벡터DB·요약을 매번 새로 만들면 **시간과 API 비용**이 큼 | 방법별 저장 경로를 나눠 디스크에 영구 저장하고, 있으면 불러오고 없을 때만 생성 |
